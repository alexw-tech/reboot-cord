...
async function startServer() {
  ...
  app.post("/api/bots", (req, res) => {
    ...
    db.prepare("INSERT INTO bots (id, user_id, name, token, status) VALUES (?, ?, ?, ?, ?)").run(id, userId, name, 'N/A', 'stopped');
  ...
  app.get("/api/bots/:id/logs", (req, res) => {
    const { id } = req.params;
    const logs = db.prepare("SELECT * FROM logs WHERE bot_id = ? ORDER BY timestamp DESC LIMIT 50").all(id);
    res.json(logs);
  });
