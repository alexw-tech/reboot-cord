...
export default function Dashboard({ onBack, user }) {
  ...
  const fetchLogs = async (id) => {
    try {
      const res = await fetch(`/api/bots/${id}/logs`, { headers: authHeaders });
      const data = await res.json();
      setLogs(data);
    } catch (err) {
  ...
  const fetchFiles = async (id) => {
    try {
      const res = await fetch(`/api/bots/${id}/files`, { headers: authHeaders });
      const data = await res.json();
      setFiles(data);
    } catch (err) {
  ...
  const fetchFileContent = async (id, filename) => {
    try {
      const res = await fetch(`/api/bots/${id}/files/${filename}`, { headers: authHeaders });
      const data = await res.json();
  ...
  const installPackages = async () => {
    ...
    try {
      const res = await fetch(`/api/bots/${selectedBotId}/pip`, { 
        method: "POST",
        headers: authHeaders
      });
      if (res.ok) {
